...
class APIs(commands.Cog):
    ...
    def REST(self, url, method="GET", headers={}, data={}, auth=None, returns="r.json()"):
        if method == "GET":
            r = requests.get(url=url, headers=headers, data=data, auth=auth)
        elif method == "POST":
            r = requests.post(url=url, headers=headers, data=data, auth=auth)
        try:
            return eval(returns)
        except:
            return None     
    ...
    async def minecraftAPI(self, ctx, *, user=None):
        ...
        if user:
            ...
            sale = await self.getMinecraftSales()
